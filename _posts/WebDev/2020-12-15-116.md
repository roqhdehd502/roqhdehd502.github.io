...
### (최대한 구현해보려 했으나 런타임 에러로 인해 구현하지 못했습니다.) ###
...
public class StackApp implements Stack {
	...
	public boolean push(String pustr) { // 스택의 톱(top)에 스트링을 push
		if (top == str.length) // 톱의 수치가 str 배열보다 같으면
			return false; // false로 리턴하여 종료 조건 성립시킨다.
		str[top++] = pustr;
		return true;
	}
